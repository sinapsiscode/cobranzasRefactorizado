                ...
                {selectedNeighborhood.clients.map((client, index) => (
                  ...
                  >
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                      ...
                      {/* Servicio contratado */}
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-1">Servicio:</h4>
                        <p className="text-sm text-gray-900 font-medium">{client.serviceName || 'Sin servicio'}</p>
                      </div>

                      {/* Meses adeudados */}
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-1">Meses adeudados:</h4>
                        <div className="text-sm text-red-600">
                          {client.owedMonths && client.owedMonths.length > 0 ? (
                            client.owedMonths.length > 2 ? (
                              <div>
                                <p className="font-medium">{client.owedMonths.length} meses</p>
                                <p className="text-xs text-gray-500">{client.owedMonths.slice(0, 2).join(', ')}...</p>
                              </div>
                            ) : (
                              <p>{client.owedMonthsText}</p>
                            )
                          ) : (
                            <p className="text-gray-500">Sin meses adeudados</p>
                          )}
                        </div>
                      </div>
