    ...
    for (let monthsBack = 8; monthsBack >= 0; monthsBack--) {
      ...
      if (dueDate >= clientInstallDate) {
        ...
        if (billingType === 'free') {
          ...
          // Clientes problemáticos: los primeros 10 tendrán 4-5 meses pendientes GARANTIZADOS
          status = 'overdue';
...
export const seedDatabase = () => {
  ...
  const seedVersion = localStorage.getItem('tv-cable:seedVersion');
  if (seedVersion === '10.2') {
    console.log('Database already seeded with validation payments data');
    return false;
  }
  ...
  try {
    ...
    localStorage.setItem('tv-cable:seedVersion', '10.2');
