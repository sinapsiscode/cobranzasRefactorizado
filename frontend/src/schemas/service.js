...
export const ServiceSchema = {
  ...
  serviceType: { type: 'enum', values: ['plan_hogar', 'plan_corporativo', 'plan_negocio'], required: true },
...
export const validateService = (data) => {
  ...
  // Campo name ahora es opcional
  if (data.name && data.name.length < 2) {
    errors.name = 'Nombre debe tener al menos 2 caracteres';
  }
  
  if (!['plan_hogar', 'plan_corporativo', 'plan_negocio'].includes(data.serviceType)) {
...
export const getServiceTypeColor = (serviceType) => {
  const colors = {
    plan_hogar: 'bg-blue-100 text-blue-800',
    plan_corporativo: 'bg-purple-100 text-purple-800',
    plan_negocio: 'bg-green-100 text-green-800'
  };
