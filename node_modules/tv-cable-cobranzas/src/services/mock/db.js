...
const DB_VERSION = '1.1'; // Incrementado para forzar regeneración después del cambio de roles
...
export class MockDB {
  ...
  init() {
    // Verificar versión de DB y regenerar si cambió
    const currentVersion = localStorage.getItem(`${DB_PREFIX}:version`);
    if (currentVersion !== DB_VERSION) {
      // Limpiar datos obsoletos y regenerar
      this.clearAllData();
  ...
  initializeCollections() {
    const collections = ['users', 'clients', 'payments', 'notifications'];
